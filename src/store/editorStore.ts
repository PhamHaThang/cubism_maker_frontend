...
export const useEditorStore = create<EditorState>((set, get) => ({
    ...
    addVoxel: (pos, color) => {
        ...
        set({
            voxels: {
                ...state.voxels,
                [key]: { position: pos, color, pieceId: state.currentPieceId },
            },
    ...
    setPieceColor: (id, color) => {
        ...
        Object.entries(state.voxels).forEach(([key, voxel]) => {
            voxels[key] = voxel.pieceId === id ? { ...voxel, color } : voxel;
        });
        ...
        set({
            ...
            voxels,
            currentColor:
                state.currentPieceId === id ? color : state.currentColor,
            undoStack: [...state.undoStack, snap].slice(-50),
            redoStack: [],
        });
    },
    ...
    newPiece: () => {
        const state = get();
        const nextId = Math.max(...state.pieces.map((p) => p.id), -1) + 1;
        const nextColor =
            PRESET_COLORS[nextId % PRESET_COLORS.length] || state.currentColor;
        set({
